    ...
    ChannelManager.ioChannels[channelid] = ioChannel = io.of('/' + channelid).on('connection', function(socket) {
        ...
        var loginUser = function() {
            ...
            if(us) {
                ...
                socket.emit('response', Collector.response(true, 'login', us));
                ...
                socket.emit('response', Collector.response(false, 'login', 'invalid_user'));
            }
        };
        //列出当前层的所有用户
        var listUser = function() {
            ...
            if(us) {
                ...
                socket.emit('response', Collector.response(true, 'list', list));
            } else {
                socket.emit('response', Collector.response(false, 'list', 'invalid_user'));
            }
        };
        //广播用户更新
        var updateUser = function() {
            var us = userSummarys[socketid];
            if(us) {
                socket.to(us.layer).broadcast.emit('response', Collector.response(true, 'update', us));
                if(us.status == 'disconnect') {
                    delete userSummarys[socketid];
                }
                console.log(us);
            } else {
                socket.emit('response', Collector.response(false, 'update', 'invalid_user'));
            }
            console.log('updateUser'/*, layfiginstances*/);
        };
        //进入频道层
        var intoLayer = function(layer) {//layer
            ...
            if('number' === typeof layerid && us) {
                if(us.layer != layerid) {
                    ...
                    socket.emit('response', Collector.response(true, 'into', us));
                    updateUser();
                    listUser();
                }
            } else if(!us) {
                socket.emit('response', Collector.response(false, 'into', 'invalid_user'));
            } else {
                socket.emit('response', Collector.response(false, 'into', 'invalid_layer'));
            }
        };
        //退出频道层，进入频道默认层
        var outLayer = function() {
            var us = userSummarys[socketid];
            if(us) {
                socket.emit('response', Collector.response(true, 'out', us));
                us.setStatus('leave');
                updateUser();
                socket.leave(us.layer);
                ...
                socket.emit('response', Collector.response(false, 'out', 'invalid_user'));
            }
        };
        var send = function(data) {
            var us = userSummarys[socketid];
            /*var tmp = Layfig.of(channelid).get(us.id);
            if(Utilities.isUndefined(tmp)) {
                tmp = [];
                Layfig.of(channelid).set(us.id, tmp);
            }
            var len = tmp.length;
            tmp[len] = data.content.content;
            console.log(data.content.content);
            Layfig.of(channelid).set(us.id, tmp);*/
            if(us) {
                socket.broadcast.emit('response', Collector.response(true, 'send', data.content));
            } else {
                socket.emit('response', Collector.response(false, 'send', 'invalid_user'));
            }
        };
