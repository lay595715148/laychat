    ...
    if(su.id == 0) {
        ...
        sservice.mread(data[sessid], function(ret) {
            if($util.isA(ret, User)) {
                ...
                Pool.factory('pool.cache.SocketUserPool').set(su);
                me.friends(function() {
                    me.update();
                });
            } else {
...
UserProcesser.prototype.friends = function(fn) {
    ...
    var friends = [], friendids = [], sup = Pool.factory('pool.cache.SocketUserPool');
        ...
        if(ret && $util.isAs(ret, User)) {
            ret.map(function(u) {
                ...
                if(suer) {
                    ...
                    su.friends[u.id] = Entity.instance('SocketUser', u);// 更新用户好友
...
UserProcesser.prototype.send = function(data) {
    ...
    if(su.id > 0) {// $logger.error(namespace.users);
        message = Data.instance('MessageData', data);
        mp = Processer.instance('MessageProcesser', su, message);
        mp.release() || mp.cache();
