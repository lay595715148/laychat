...
UserProcesser.prototype.init = function(data) {
    var me = this;
    ...
    socket.on('request', function(data) {
        ...
        if(me.user) {
            me.user.setStatus('disconnect');
            me.update();
        }
        $logger.debug('do disconnect');
    }).on('reconnecting', function() {
        if(me.user) {
            me.user.setStatus('reconnecting');
            me.update();
        }
...
UserProcesser.prototype.list = function() {
...
UserProcesser.prototype.into = function(layer) {
...
UserProcesser.prototype.out = function() {
...
UserProcesser.prototype.send = function(data) {
    ...
    if($util.isObject(su)) {//$logger.error(namespace.users);
        mp = new MessageProcesser(socket, su);
