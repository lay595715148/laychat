    ...
    if(su.id == 0) {
        ...
        sservice.mread(data[sessid], function(ret) {
            if(ret instanceof User) {
                ...
                //Pool.factory('SocketUserPool').load();
                Pool.factory('SocketUserPool').push(su);
                //Pool.factory('SocketUserPool').save();
                me.friends();
                //me.update();
            } else {
...
UserProcesser.prototype.friends = function() {
    ...
    var friends = [], friendids = [], sus, fservice, uservice, aynsc = new Async(), rspdata;
    if(su.id > 0) {
        ...
        aynsc.push(fservice.readByUser, [su.id], fservice, function(ret) {
            if(ret && $util.isArray(ret)) {
                ...
                //Pool.factory('SocketUserPool').load();
                sus = Pool.factory('SocketUserPool').gets(friendids);$logger.error(sus.length);
                sus.map(function(item) {
                    rspdata = Collector.response(true, 'login', item.toSocketUserSummary());
                    item.socket.emit('response', rspdata);
                });
