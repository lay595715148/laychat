...
UserSummary.prototype.setSocket = function(socket) {
    if('object' === typeof socket && 'string' === typeof socket.id)
        this.socket = socket.id;
    else if('string' === typeof socket)
        this.socket = socket;
};
/**
...
UserSummary.prototype.setChannel = function(channel) {
    if('object' === typeof channel && 'number' === typeof channel.id)
        this.channel = channel.id;
    else if('number' === typeof channel)
...
UserSummary.prototype.setLayer = function(layer) {
    if('object' === typeof layer && 'number' === typeof layer.id)
        this.layer = layer.id;
    else if('number' === typeof layer)
        this.layer = layer;
};
/**
 * user current status
...
UserSummary.prototype.setStatus = function(status) {
    if('number' === typeof status || 'string' === typeof status)
        this.status = status;
};
/**]
 * 
