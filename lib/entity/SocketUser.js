...
var SocketUserSummary = $.data.SocketUserSummary;

/**
 * 用户总述对象
 */
function SocketUser(id, name, nick, socket, channel, layer, friends, status) {
    var _id = 0, _name = '', _nick = '', _socket = {}, _channel = {}, _layer = 0, _friends = {}, _status = '';
    ...
    this.__defineSetter__('friends', function(friends) {
        if($util.isObject(friends) && $util.isOs(friends, SocketUser))
            _friends = friends;
    });
    ...
    this.__defineGetter__('friends', function() {
        return _friends;
    });
    ...
    this.friends = friends;// 所在好友
    this.status = status;// 当前状态
}

$util.inherits(SocketUser, Entity);
 ...
 *            {Array<SocketUser>}
 */
SocketUser.prototype.setFriends = function(friends) {
 ...
 * @returns {Array}
 */
SocketUser.prototype.getFriends = function() {
    return this.friends;
};
/**
 * @returns {String}
 ...
 * @abstract
 */
SocketUser.key = SocketUser.prototype.key = function() {
    return 'id';
};
