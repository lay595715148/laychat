...
Authorize.prototype.launch = function() {
    ...
    if(checked) {$logger.info($util.toString(suser));
        var async = new Async();
...
Authorize.prototype.submit = function() {
    ...
    var get = scope.get(), post = scope.post(), request = scope.request(), session = scope.session();
    var suser = this.prefacer.user?this.prefacer.user:{};//session memecache user
    var response_type = request.response_type || 'code';
    ...
    if(other) {
        ...
        me.prefacer.remove();
        success('/authorize?client_id=' + clientId + '&redirect_uri=' + encodeURIComponent(redirectURI));
    } else if(register) {
        success('/register');
    } else if(checked) {
        //打开检测登录客户端任务
