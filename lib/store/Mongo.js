 ...
 * @returns
 */
Mongo.prototype.nextSequence = function(name, fn) {
    var me = this;
    ...
    name = table + '.' + name;$logger.info(name);

    this.connect(function() {
        link.collection(sequence).findAndModify({ name : name }, [], { $inc : { seq : 1 } }, { 'new':true }, function(err, result) {
            assert.equal(null, err);
            me.result = result;
            $logger.info(sequence, name, result.seq);
